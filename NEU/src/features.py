...
def add_feature_user_tip_lags(orders: pd.DataFrame, tips: pd.DataFrame, n_lags: int = 3) -> pd.DataFrame:
    """
    Creates lagged tip features for each user.

    Args:
        orders (pd.DataFrame): The orders dataframe.
        tips (pd.DataFrame): The tips dataframe.
        n_lags (int): The number of previous tips to include as features.

    Returns:
        pd.DataFrame: A dataframe with [order_id, tip_lag_1, ...].
    """
    user_tip_history = pd.merge(orders[['order_id', 'user_id', 'order_date']], tips, on="order_id", how="left")
    user_tip_history.sort_values(["user_id", "order_date"], inplace=True)
    
    for i in range(1, n_lags + 1):
        # Create lagged features based on past tips for each user
        user_tip_history[f'tip_lag_{i}'] = user_tip_history.groupby("user_id")["tip"].shift(i)

    # For the model, we can fill NaNs. A simple fill with 0.5 (neutral prob) is a reasonable start.
    lag_cols = [f'tip_lag_{i}' for i in range(1, n_lags + 1)]
    user_tip_history[lag_cols] = user_tip_history[lag_cols].fillna(0.5)

    return user_tip_history[['order_id'] + lag_cols]
...
def add_feature_avg_tip_rate_department(order_products_denormalized, orders, tips_public, default_tip_rate=0.500111):
    ...
    merged['order_count'] = merged.groupby('department', observed=False).cumcount().astype('int32')
    merged['tip_cumsum_before'] = merged.groupby('department' , observed=False)['tip'].cumsum() - merged['tip']
    ...
    order_tip_rate = merged.groupby('order_id' , observed=False)['avg_tip_rate_before'].mean().reset_index(name='order_dept_tip_rate')
...
def combine_all_features(orders, order_products_denormalized, tips_public,
                         forecast_df: pd.DataFrame = None, default_tip_rate=0.500111):
    """
    Combines all engineered features into a DataFrame at the order_id level.
    """
    # --- 1. BASE DATAFRAME ---
    # Start with the base dataframe containing all order_ids and user_ids.
    # This will be our main 'result' dataframe that we progressively build upon.
    result = orders[['order_id', 'user_id', 'order_date']].copy()
    ...
    # --- 2. MERGE ORDER-LEVEL FEATURES ---
    print("Merging order-level features...")
    order_features_to_merge = [
    ...
    for feature_df in order_features_to_merge:
        result = result.merge(feature_df, on='order_id', how='left')

    # --- 3. MERGE USER-LEVEL FEATURES ---
    print("Merging user-level features...")
    user_features_to_merge = [
    ...
    for feature_df in user_features_to_merge:
    ...
    global_mean_tip = tips_public['tip'].mean()
    if 'forecasted_hourly_tip_share' in result.columns:
        result['forecasted_hourly_tip_share'].fillna(global_mean_tip, inplace=True)
    else:
        result['forecasted_hourly_tip_share'] = global_mean_tip
        
    # --- 5. ADD TARGET VARIABLE AND FINALIZE ---
    print("Adding target variable and finalizing.")
    result = result.merge(tips_public[['order_id', 'tip']], on='order_id', how='left')
    
    # Final cleanup and downcasting
    result = downcast_dtypes(result.drop(columns=['order_date'])) # Drop date column as it's no longer needed
